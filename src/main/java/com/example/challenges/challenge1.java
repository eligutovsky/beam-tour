...
public class challenge1 {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(challenge1.class);
    ...
    static PCollection<Double> getAboveCost(PCollection<Double> input) {
        return input.apply("Get Above Cost", Filter.greaterThanEq(FIXED_COST));
    }

    static PCollection<Double> getBelowCost(PCollection<Double> input) {
        return input.apply("Get Below Cost", Filter.lessThan(FIXED_COST));
    }

    // Define the type PCollection<?>
    static PCollection<KV<String, Double>> setKeyForCost(PCollection<Double> input, String key) {
        return input.apply(key, WithKeys.<String, Double>of(key));
    }
